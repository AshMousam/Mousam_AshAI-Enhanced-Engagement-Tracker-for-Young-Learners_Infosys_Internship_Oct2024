...
try:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"screenshots/mousam_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for mousam with screenshot
                ...
                # Draw a rectangle around the face and label it as Mousam
                for face_location in face_locations:
                    top, right, bottom, left = face_location
                    cv.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
                    cv.putText(frame, 'mousam', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not mousam'
            cv.putText(frame, 'Not Mousam', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
    ...
    if not df.empty:
        df.to_excel('Mousam_attendance_with_screenshots.xlsx', index=False)
        print("Attendance and screenshots saved to 'Mousam_attendance_with_screenshots.xlsx'.")
